...
public:
    ...
    virtual void encode(const std::vector<AbstractProtocol::s_ptr>& message, TcpBuffer::s_ptr out_buffer) = 0; 
    // 将buffer中的字节流转化为message对象 
    virtual void decode(std::vector<AbstractProtocol::s_ptr>& message, const TcpBuffer::s_ptr in_buffer) = 0; 
